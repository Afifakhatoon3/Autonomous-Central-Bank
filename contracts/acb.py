...
class AutonomousCentralBank(gl.Contract):
    ...
    def __init__(self) -> None:
        ...
        wiki_hash = gl.crypto.keccak256(
            "en.wikipedia.org".encode()
        )
    ...
    def fetch_data(
        ...
        self.observation_counter = (
            self.observation_counter + u256(1)
        )
    ...
    def propose_policy(
        ...
        if (
            now - int(obs.fetched_at)
            > int(self.freshness_seconds)
        ):
    ...
    def get_bond_requirements(self) -> dict:
        return {
            "base_min_bond_wei": int(
                self.base_min_bond
            ),
            "current_min_bond_wei": (
                self._current_min_bond()
            ),
            "proposal_count": int(
                self.proposal_counter
            ),
