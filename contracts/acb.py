...
class AutonomousCentralBank(gl.Contract):
    ...
    def __init__(self) -> None:
        ...
        self.trusted_domains["en.wikipedia.org"] = True
        self.trusted_domains["wikipedia.org"] = True
...
def remove_trusted_domain(self, domain: str) -> None:
    self._require_admin()
    self.trusted_domains[domain] = False
...
def fetch_data(self, sources: str) -> u256:
    ...
    if self.enforce_allowlist:
        for url in urls:
            ...
            if not self.trusted_domains.get(domain, False):
...
def propose_policy(self, observation_id: u256, direction: str) -> u256:
...
@gl.public.view
def get_trusted_domain(self, domain: str) -> bool:
    return self.trusted_domains.get(domain, False)
